...
def simulation(D, A, t, dt, endtime, mesh, initial):
    ...
    sp = { "linear_solver":  "cg", "preconditioner": "jacobi" }
    fcp = { "optimize": True }
    ...
    while t < endtime:
        ...
        if adv:
            ...
            t_a_a.stop()
            t_a_s.start()
            solve(Mass==adv_rhs, u1, solver_parameters=sp, form_compiler_parameters=fcp)
            t_a_s.stop()
        ...
        if diff:
            ...
            t_d_a.stop()
            t_d_s.start()
            solve(diff_matrix==diff_rhs, u1, solver_parameters=sp, form_compiler_parameters=fcp)
            t_d_s.stop()
        #if save_output:
        #    out_file << (u1, t)
